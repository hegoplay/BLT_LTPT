...
public static OrderDAO instance = new OrderDAO();
	...
	public Order findById(String id) {
		...
		return StorageManager.createQuery(cq).getSingleResult();
	}
	...
	public Set<OrderDetail> getODSetByOrder(Order order) {
		...
		cq = cq.select(root).where(where);
		return StorageManager.createQuery(cq).getResultList().stream().collect(java.util.stream.Collectors.toSet());
	}
